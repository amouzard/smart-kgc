...
class Rotate3D(KGEModel):
    def __init__(self, num_entity, num_relation, hidden_dim, gamma, p_norm):
        ...
        self.choice_embedding = nn.Parameter(torch.zeros(num_relation, 1))
        nn.init.uniform_(
...
class RotatE(KGEModel):
    def __init__(self, num_entity, num_relation, hidden_dim, gamma, p_norm):
        ...
        self.ellipse = nn.Parameter(torch.rand((2 * hidden_dim)), requires_grad=True)
    ...
    def func(self, head, rel, tail, batch_type, relation_choice):
        ...
        if batch_type == BatchType.HEAD_BATCH:
            re_score = re_tail * re_relation + im_tail * im_relation - re_head
            im_score = -re_tail * im_relation + im_tail * re_relation - im_head
        else:
